...
const getNum = (string, which) => {
        if (string) {
                ...
                if (which === 'last') {
                        const matches = string.split('')?.reverse()?.join('')?.match(reverseRegex)?.[0]?.join('').split('').reverse('').join('');

                        const x = string.split('').reverse().join('').match(reverseRegex);

                        console.log(matches);

                        return validDigit?.[matches?.[matches.length - 1]];
                }
        }
};
...
const calibrateString = (string) => {
        ...
        // will need to prepare case if indexoffirst == indexoflast
